...
import { BEGIN_STEP, BEGIN_STEP_BPMN, START_BPMN, BEGIN_STEP_HOVER /*, BEGIN_STEP_SELECTED*/ } from './MapImages';
...
class BeginStep extends GenericStep {
	...
	handleMouseLeave = (e: any) => {
		let selection = d3.select<any, any>(this.refs.image);
		selection.attr('href', this.props.isBPMNView === true ? BEGIN_STEP_BPMN : BEGIN_STEP);
	};

	render() {
		const { selectStep, stopDragging, showStepProps, isBPMNView, localizedName } = this.props;
		...
		//	fontSize: GenericStep.defaultProps.style.fontSize
		...
		return (
			<g onMouseDown={selectStep} onMouseUp={stopDragging} onDoubleClick={showStepProps}>
				{showRect === true ? (
					<rect
						x={Location.X}
						y={Location.Y}
				...
				{isBPMNView === true ? (
					<g>
						<image
							...
							ref="startImage"
						/>
						<text x={this.getBPMNStartX() + 4} y={this.getTextY()} fill="black" ref="text">
							Start
						</text>
					</g>
				) : (
