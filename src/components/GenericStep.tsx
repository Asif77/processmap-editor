...
import { StepState, TaskStatuses, TaskSubStatuses, RecipientTypes } from './constant';
import {
	ACTIVE,
	COMPLETED_HALF,
	REASSIGN,
...
export default class GenericStep extends React.Component<IStepProps, any> {
	...
	getStateImageX = () => {
		...
		x += (ImageConst.CX + Location.X - x) / 2.0 - (ImageConst.STATEIMAGEWIDTH >> 1);
	...
	getTextX = () => {
		...
		if (textSize !== null) {
			if (ImageConst.CX < textSize.width) textX -= (textSize.width - ImageConst.CX) / 2.0;
	...
	getTextY = () => {
		...
		return textSize !== null ? Location.Y + ImageConst.CY + textSize.height : Location.Y;
	};
	...
	togglePopup = () => {
		...
		if (tooltipDiv !== null && tooltipDiv.style.display === 'block') {
	...
	displayPopup = () => {
		if (this.props.stepContext) {
			...
			if (strRecipient) {
				...
				let tooltipDiv = document.getElementById('tooltip');
				if (tooltipDiv !== null) {
					tooltipDiv.innerHTML = `<p class='title'>${title}</p><div class='line'></div><p class='description'>${description}</p>`;
					tooltipDiv.style.top = (this.getImageY() + 64) * translation.scaleFactor + translation.y + 'px';
					tooltipDiv.style.left = this.getImageX() * translation.scaleFactor + translation.x + 'px';
	...
	getStateImage = () => {
		if (this.props.stepContext) {
			...
			if (ListTaskStatus && ListTaskStatus.length > 0) {
				return this.GetStatusImage(this.GetStepState(ListTaskStatus[0], ListTaskSubStatus[0]));
			}
		}
	...
	GetStepState = (status: number, substatus: number) => {
		...
		let URGENT_REASSIGNED = URGENT | TaskSubStatuses.TASK_STATUS_ACTIVE_REASSIGNED;
		let URGENT_RESUBMITTED = URGENT | TaskSubStatuses.TASK_STATUS_ACTIVE_RESUBMITTED;

		let OVERDUE = TaskSubStatuses.TASK_STATUS_ACTIVE_OVERDUE;
		let OVERDUE_CONFERED = OVERDUE | TaskSubStatuses.TASK_STATUS_ACTIVE_CONFERED;
		let OVERDUE_RETURNED = OVERDUE | TaskSubStatuses.TASK_STATUS_ACTIVE_RETURNED;
		let OVERDUE_REASSIGNED = OVERDUE | TaskSubStatuses.TASK_STATUS_ACTIVE_REASSIGNED;
		let OVERDUE_RESUBMITTED = OVERDUE | TaskSubStatuses.TASK_STATUS_ACTIVE_RESUBMITTED;

		if (status === TaskStatuses.TASK_STATUS_ACTIVE) {
			if (this.ContainsBits(substatus, URGENT_REASSIGNED)) return StepState.ReassignedLate;
			if (this.ContainsBits(substatus, OVERDUE_REASSIGNED)) return StepState.ReassignedOverDue;
			if (this.ContainsBits(substatus, TaskSubStatuses.TASK_STATUS_ACTIVE_REASSIGNED))
				return StepState.Reassigned;

			if (this.ContainsBits(substatus, URGENT_RETURNED)) return StepState.ReturnedLate;
			if (this.ContainsBits(substatus, OVERDUE_RETURNED)) return StepState.ReturnedOverDue;
			if (this.ContainsBits(substatus, TaskSubStatuses.TASK_STATUS_ACTIVE_RETURNED))
				return StepState.ActiveReturned;

			if (this.ContainsBits(substatus, URGENT_CONFERED)) return StepState.ConferredLate;
			if (this.ContainsBits(substatus, OVERDUE_CONFERED)) return StepState.ConferredOverDue;
			if (this.ContainsBits(substatus, TaskSubStatuses.TASK_STATUS_ACTIVE_CONFERED)) return StepState.Conferrred;

			if (this.ContainsBits(substatus, URGENT_RESUBMITTED)) return StepState.ResubmittedLate;
			if (this.ContainsBits(substatus, OVERDUE_RESUBMITTED)) return StepState.ResubmittedOverDue;
			if (this.ContainsBits(substatus, TaskSubStatuses.TASK_STATUS_ACTIVE_RESUBMITTED))
		...
		if (status === TaskStatuses.TASK_STATUS_COMPLETED) {
			if (this.ContainsBits(substatus, TaskSubStatuses.TASK_STATUS_COMPLETE_INCIDENT_COMPLETE))
		...
		if (status === TaskStatuses.TASK_STATUS_INQUEUE) {
			...
			if (this.ContainsBits(substatus, OVERDUE)) return StepState.InQueueOverDue;
