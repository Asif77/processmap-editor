class MapApi {
	public static getMap() {
		return new Promise((resolve, reject) => {
			setTimeout(() => {
				fetch('http://localhost/UltimusProcessConfig/Api/v1.0/UltimusProcess/GetProcess')
					.then((response) => {
	...
	public static getProcessMap2(server: string, processName: string, version: number, incident: number) {
	...
	public static async getLocalizedData(server: string, serverId: string, processName: string, language: string) {
		try {
			const api =
			...
			const data = response.ok === true && response.status === 200 ? await response.json() : [];
			console.log(data);
			return data;
		} catch (error) {
			console.log(error);
	...
	public static async getProcessMap(server: string, processName: string, version: number, incident: number) {
		try {
			const api = `${server}/UltimusProcessConfig/Api/v1.0/UltimusProcess/GetProcessMap/?processName=${processName}&version=${version}&incident=${incident}`;
			const response = await fetch(api);
	...
	public static async loadPlaybackHistory(server: string, processName: string, version: number, incident: number) {
		try {
			const api = `${server}/UltimusProcessConfig/Api/v1.0/UltimusProcess/GetPlaybackHistory/?processName=${processName}&version=${version}&incident=${incident}`;
			const response = await fetch(api);
			const data = await response.json();
			console.log(data);
			return data;
	...
	public static async getLocalizedData2(server: string, serverId: string, processName: string, language: string) {
		const mode = 'no-cors';
		const method = 'GET';
		const headers = {
